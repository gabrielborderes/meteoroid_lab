...
for file_name in tqdm(file_list):
    with h5py.File(file_name, "r") as hf:
        if first:
            ...
            atmp = hf["a"][:]
            ecc = hf["e"][:]
            a = np.zeros((atmp.shape[0],atmp.shape[1]*len(file_list)))
        else:
            atmp = hf["a"][:]
            ecc = hf["e"][:]
    ...
    for t_tmp in range(atmp.shape[1]):
        for bd in range(0,len(index)):
            if ecc[bd][t_tmp] < 1.:
                a[bd][t_tmp + atmp.shape[1]*n_file] = atmp[bd][t_tmp]
            else:
                a[bd][t_tmp + atmp.shape[1]*n_file] = None
...
# # Testando se "_cl_" está nos itens
# for item in index:
#     print(f"Verificando: {repr(item)} -> '_cl_' in item? { '_cl_' in item }")
...
if active_cl:
    ...
    for bd in range(ni_met,len(index)):
        ...
        ax.plot(time/year, a[bd]/au.value, color=color)
    ...
    for bd in range(ni_met,len(index)):
        ...
        if met_group [bd-ni_met] == "G1":
            ax.plot(time/year, a[bd]/au.value, color="blue")
        else:
