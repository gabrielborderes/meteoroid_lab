...
for file_name in tqdm(file_list):
    ...
    for i in range(a_tmp.shape[1]):
        for j in range(a_tmp.shape[0]):
            if j != 0:
                a[j,t_s+i] = a_tmp[j,i]*(1.-e_tmp[j,i])
    t_s = t_e
...
for i in range(ni_met,a.shape[0]):
    ax1.plot(t / year, a[i, :], color='blue')

#ax1.scatter(t / year, a[14, :],s=1, color='black')
ax1.plot(t / year, a[14, :], color='black')

line_value = 0.974

ax1.axhline(y=line_value, color='r', linestyle='--')
...
#ax2.set_yticks([])
plt.savefig(f"data/q-b.png", bbox_inches="tight")
