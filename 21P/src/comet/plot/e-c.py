...
for file_name in tqdm(file_list):
    ...
    t_e = int(t_e +  e_tmp.shape[1])
    if t_e > len(t):
        e[1:,t_s:len(t)] = e_tmp[1:,:len(t)-t_s]
        break
...
e_len = np.zeros((len(bins) - 1))
for i in range(len(bins) - 1):
    e_len[i] = bins[i + 1]/2. + bins[i]/2.


for j in tqdm(range(e.shape[1])):

    for i in range(ni_met,e.shape[0]):
        e_eval = e[i, j]

        for k in range(len(bins) - 1):
            if bins[k] <= e_eval < bins[k + 1]:
                heatmap_data[k, j] += 1
...
extent = [t[0]/year, t[-1]/year, e_len[-1], e_len[0]]  # inverte y para bater com heatmap
...
#ax1.plot(t / year, e[14, :], color='black')

ax1.scatter(t / year, e[14, :],s=0.2, color='black')
ax1.axhline(y=0.7396, color='r', linestyle='--')
...
#ax2.set_yticks([])
plt.savefig(f"data/e_log-zoom.png", bbox_inches="tight")
