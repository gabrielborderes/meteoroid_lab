...
for file_name in tqdm(file_list):
    ...
    t_e = int(t_e +  o_tmp.shape[1])
    if t_e > len(t):
        om[1:,t_s:len(t)] = o_tmp[1:,:len(t)-t_s]
        break


    om[1:,t_s:t_e] = o_tmp[1:,:]
    t_s = t_e
...
o_len = np.zeros((len(bins) - 1))
for i in range(len(bins) - 1):
    o_len[i] = bins[i + 1]/2. + bins[i]/2.


for j in tqdm(range(om.shape[1])):

    for i in range(ni_met,om.shape[0]):
        o_eval = om[i, j]

        for k in range(len(bins) - 1):
            if bins[k] <= o_eval < bins[k + 1]:
                heatmap_data[k, j] += 1
...
extent = [t[0]/year, t[-1]/year, o_len[-1], o_len[0]]  # inverte y para bater com heatmap
...
ax1.set_xlabel('Time (year)')
ax1.set_ylabel('longitude of ascending node (degrees)')


#ax1.plot(t / year, om[14, :], color='black')
ax1.scatter(t / year, om[14, :], s=0.2, color='black')
ax1.axhline(y=142, color='r', linestyle='--')
...
#ax2.set_yticks([])
plt.savefig(f"data/Omega-long-zoom.png", bbox_inches="tight")
