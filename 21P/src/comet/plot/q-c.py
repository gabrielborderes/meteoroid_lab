...
for file_name in tqdm(file_list):
    ...
    if t_e > len(t):
        for i in range(len(t)-t_s):
            for j in range(a_tmp.shape[0]):
                if j != 0:
                    q[j,t_s+i] = a_tmp[j,i]*(1.-e_tmp[j,i])
        break


    for i in range(a_tmp.shape[1]):
        for j in range(a_tmp.shape[0]):
            if j != 0:
                q[j,t_s+i] = a_tmp[j,i]*(1.-e_tmp[j,i])
    t_s = t_e
...
q_len = np.zeros((len(bins) - 1))
for i in range(len(bins) - 1):
    q_len[i] = bins[i + 1]/2. + bins[i]/2.


for j in tqdm(range(q.shape[1])):

    for i in range(ni_met,q.shape[0]):
        q_eval = q[i, j]

        for k in range(len(bins) - 1):
            if bins[k] <= q_eval < bins[k + 1]:
                heatmap_data[k, j] += 1
...
extent = [t[0]/year, t[-1]/year, q_len[-1], q_len[0]]  # inverte y para bater com heatmap
...
#ax1.plot(t / year, q[14, :], color='black')

ax1.scatter(t / year, q[14, :],s=0.2, color='black')
ax1.axhline(y=0.974, color='r', linestyle='--')
...
#ax2.set_yticks([])
plt.savefig(f"data/q_log-zoom.png", bbox_inches="tight")
